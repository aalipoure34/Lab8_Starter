describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let actualText = await text['_remoteObject'].value
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    //Revert the button from previous step
    let product = await page.$("product-item")
    let shadow = await product.getProperty("shadowRoot")
    let button = await shadow.$("button")
    await button.click()
    ...
    let realText = await text["_remoteObject"].value
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    expect(buttonVal).toBe(true);
    
    let cart = await page.$("#cart-count")
    let cartText = await cart.getProperty("innerText")
    let realText = await cartText["_remoteObject"].value

    if(realText != "20") {
      cartCount = false;
    }

    expect(cartCount).toBe(true);
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    let cartArr = await page.evaluate("window.localStorage.cart")
